...
#define PI 3.14159
#define MODULUS_OF_ELASTICITY 200000
#define SHEAR_MODULUS 80000
#define YIELD_STRESS 300

char output[MAX_RESULT_SIZE];
char steelSectionName[STEEL_SECTION_NAME_MAX_LENGTH];
int steelSectionNameSize = 0;
uintmax_t Zx; //Effective Section Modulus.
uintmax_t Iy; //The second moment of area about the minor y axis.
uintmax_t J; //Section Torsion constant.
uintmax_t Iw; //Section warping constant.
...
double calculateMo(int lengthInMetre) {
  int lengthInMM = lengthInMetre * 1000;
  uintmax_t EIy = MODULUS_OF_ELASTICITY * Iy; //Modulus of Elasticity * The second moment of area about the minor y axis.
  uintmax_t EIw = MODULUS_OF_ELASTICITY * Iw; //Modulus of Elasticity * Section warping constant.
  double term1 = (squared(PI) * EIy)/(squared(lengthInMM));
  uintmax_t GJ = SHEAR_MODULUS * J;
  double term2 = (squared(PI) * EIw)/(squared(lengthInMM));
  double Mo = sqrt(term1 * (GJ + term2));
  return Mo;
}

double calculateAlpha(uintmax_t Ms, double Mo) {
  double MsByMo = ((double)Ms)/Mo;
  double MsByMoSquare = squared(MsByMo);
  double alpha = 0.6 * (sqrt(MsByMoSquare + 3) - MsByMo);
  return alpha;
}

void calculateArray() {
  uintmax_t Ms = YIELD_STRESS * Zx; //section capacity.
  ...
  for(int udl = 1; udl<=10; ++udl) {
    ...
    for(int le = 0;le<=10; ++le) {
      if(le == 0) {
        int l_le = 5;
        Moment = (udl * squared(l_le))/8;
        MomentByMs = (Moment * pow(10,6))/(double)Ms;
        values[udl-1][le] = MomentByMs;
      }
      else {
        Moment = (udl * squared(le))/8;
        Mo = calculateMo(le);
        alpha = calculateAlpha(Ms,Mo);
        Mb = alpha * Ms;
        MomentByMs = (Moment * pow(10,6))/(double)Ms;
